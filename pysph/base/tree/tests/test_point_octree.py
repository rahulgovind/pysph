...
class Octree2DTestCase(unittest.TestCase):
    def setUp(self):
        use_double = False
        self.N = 3000
        ...
        self.octree = OctreeGPU(pa, radius_scale=1., use_double=use_double,
                                leaf_size=32, dim=2)
        self.leaf_size = 32
        self.octree.refresh(np.array([0., 0.]), np.array([1., 1.]),
                            np.min(pa.h))
        self.pa = pa
...
class PointTreeTestCase(unittest.TestCase):
    def setUp(self):
        use_double = False
        self.N = 3000
        pa = _gen_uniform_dataset_2d(self.N, 0.2, seed=0)
        self.octree = PointTree(pa, radius_scale=1., use_double=use_double,
                                leaf_size=32, dim=2)
        self.leaf_size = 32
        self.octree.refresh(np.array([0., 0.]), np.array([1., 1.]),
                            np.min(pa.h))
