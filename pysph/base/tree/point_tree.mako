...
<%def name="preamble(data_t)" cached="True">
    ...
    #define NORM2_1(X, Y) ((X)*(X))
    #define NORM2_2(X, Y) ((X)*(X) + (Y)*(Y))
    #define NORM2_3(X, Y, Z) ((X)*(X) + (Y)*(Y) + (Z)*(Z))
...
<%def name="reorder_particles_args(powdim)" cached="False">
    int *pids,
    ulong *sfc, int *cids,
    char *seg_flag,
    uint2 *pbounds,
    int *offsets,
    uint${powdim} *octant_vector,
...
<%def name="reorder_particles_src(powdim)" cached="False">
...
<%def name="set_node_data_args(powdim)", cached="False">
    int *offsets_prev, uint2 *pbounds_prev,
    int *offsets, uint2 *pbounds,
    char *seg_flag, uint${powdim} *octant_vector,
    uint csum_nodes, uint N
</%def>

<%def name="set_node_data_src(powdim)", cached="False">
    ...
    uint${powdim} octv = octant_vector[pbound_here.s1 - 1];

    % for i in range(powdim):
        % if i == 0:
